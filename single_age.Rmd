...
library(zoo)
covars.smooth <- apply(covars,2, function(x)  rollapply(x, align='right',FUN=mean, width=12,  partial=T) )
...
covars.log <- covars.log[, colSums(covars.log)>0]
  ...
  df.mod <- cbind.data.frame('J12_J18_prim'=a2$J12_J18_prim, 'index'=a2$index, 'spl0'=a2$spl0, 'spl1'=a2$spl1,
                             'pc1'=pcs[,1],'pc2'=pcs[,2],'pc3'=pcs[,3],'pc4'=pcs[,4], 'monthdate2'=a2$monthdate2)
  
  mod1 <- glm.nb( J12_J18_prim ~ index+ spl0+spl1 + offset(pc1) ,     data=df.mod)  
